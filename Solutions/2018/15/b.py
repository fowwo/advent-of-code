...
for line in file:
	...
	for x in line.strip():
		if x == '#':
			...
			elves.append(Unit(x, r, c, 3))
		elif x == 'G':
			goblins.append(Unit(x, r, c, 3))
		c += 1
	r += 1
file.close()
...
ap = 4
units = []
while len(units) != len(elves) or units[0].type != 'E':
	units = [ Unit(x.type, x.r, x.c, ap) for x in elves ] + [ Unit(x.type, x.r, x.c, x.ap) for x in goblins ]
	round = 0
	end = False
	while not end:
