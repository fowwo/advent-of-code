...
while line != '':
	initialImmuneSystem.append(parseUnitGroup(line, "immuneSystem"))
	line = file.readline().strip()
file.readline()
line = file.readline().strip()
while line != '':
	initialInfection.append(parseUnitGroup(line, "infection"))
	line = file.readline().strip()
file.close()
...
while True:
	boost += 1
	immuneSystem = [ UnitGroup(x.team, x.units, x.health, x.damageType, x.damage, x.weak, x.immune, x.initiative) for x in initialImmuneSystem ]
	infection = [ UnitGroup(x.team, x.units, x.health, x.damageType, x.damage, x.weak, x.immune, x.initiative) for x in initialInfection ]
	for x in immuneSystem: x.damage += boost
	...
	while len(immuneSystem) != 0 and len(infection) != 0:
		...
		while i < len(groups):
			...
			if b:
				if a.getFutureDamage(b) >= b.health: stalemate = False
				killed = a.attack(b)
				if killed:
	...
	if len(immuneSystem) > 0 and not stalemate: break

s = 0
for x in immuneSystem: s += x.units
print(s)
