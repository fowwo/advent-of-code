...
public class _2b {
	public static void main(String[] args){
		...
		while (intArr.get(0) != 19690720){

			// Convert from String array to Integer ArrayList (resetting)
			intArr.clear();
			for (int i = 0; i < strArr.length; i++){
				intArr.add(Integer.parseInt(strArr[i]));
			}

			// Increment noun and verb (brute-force)
			noun++;
			if (noun == 100){
				noun = 0;
				verb++;
		...
		System.out.printf("Noun: %d, Verb: %d\n", noun, verb);
		System.out.printf("100 * noun + verb = %d\n", 100 * noun + verb);
	}
}
