...
function buildPath(command){
	var line = [];
	var x = 0;
	var y = 0;
	for (var i = 0; i < command.length; i++){
		var magnitude = Number(command[i].slice(1));
		if (command[i][0] == "R"){
			line.push(new Line(x,x + magnitude,y,y));
			x += magnitude;
		} else if (command[i][0] == "U"){
			line.push(new Line(x,x,y,y + magnitude));
			y += magnitude;
